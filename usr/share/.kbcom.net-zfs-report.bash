...
zfs_write_arcstatslog()
{
 ...
 echo "$LOCAL_STRING_DATETIME" > "$LOCAL_FILE_ARCSTATSLOG"

 for LOCAL_ITEMKEYSTRING_ARCSTATS in "${!GLOBAL_ARRAY_ARCSTATS[@]}"
 do
  echo "$LOCAL_ITEMKEYSTRING_ARCSTATS ${GLOBAL_ARRAY_ARCSTATS[$LOCAL_ITEMKEYSTRING_ARCSTATS]}" >> "$LOCAL_FILE_ARCSTATSLOG"
 done
}
...
zfs_print_arcstats()
{
 ...
 do
  ...
  then
   ...
   if [ "$LOCAL_NUMBER_HITSVALUE" -ne 0 ] && [ "$LOCAL_NUMBER_MISSESVALUE" -ne 0 ] 2> /dev/null
   then
...
zfs_print_arcstatslog()
{
 ...
 local LOCAL_RATIO_HITSLOG
 local LOCAL_RATIO_MISSESLOG
 ...
 do
  ...
  then
   ...
   if [ "$LOCAL_NUMBER_HITSLOGVALUE" -ne 0 ] && [ "$LOCAL_NUMBER_MISSESLOGVALUE" -ne 0 ] 2> /dev/null
   then
    let "LOCAL_RATIO_HITSLOG=($LOCAL_NUMBER_HITSLOGVALUE*10000)/($LOCAL_NUMBER_HITSLOGVALUE+$LOCAL_NUMBER_MISSESLOGVALUE)"
    let "LOCAL_RATIO_MISSESLOG=($LOCAL_NUMBER_MISSESLOGVALUE*10000)/($LOCAL_NUMBER_HITSLOGVALUE+$LOCAL_NUMBER_MISSESLOGVALUE)"
   else
    LOCAL_RATIO_HITSLOG=""
   ...
   then
    ...
    if [ ! -z "$LOCAL_RATIO_HITSLOG" ]
    then
     echo " (${LOCAL_RATIO_HITSLOG:0:-2}.${LOCAL_RATIO_HITSLOG: -2}%)"
    else
     echo
    fi
    ...
    if [ ! -z "$LOCAL_RATIO_MISSESLOG" ]
    then
     echo " (${LOCAL_RATIO_MISSESLOG:0:-2}.${LOCAL_RATIO_MISSESLOG: -2}%)"
    else
     echo
    fi
   fi
